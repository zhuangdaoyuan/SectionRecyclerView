...
import com.easyandroid.sectionadapter.TestAdapter;
...
public class SectionedGridDivider extends RecyclerView.ItemDecoration {
    ...
    private boolean isDraw(RecyclerView parent, View itemView, int totalCount) {
        ...
        if (((TestAdapter) parent.getAdapter()).isHasHeader()) {
            count = 2;
            itemPosition--;
            if (((TestAdapter) parent.getAdapter()).isHasHeaderDivider()) {
                if (itemPosition < 0) {
                    return false;
                }
        ...
        //要除去footer占有的一个位置以及header占有的位置（如果有）
        if (totalCount > count && itemPosition < totalCount - count) {
    ...
    private void drawHorizontal(Canvas c, RecyclerView parent) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (isDraw(parent, child, totalCount)) {
                ...
                int left = child.getLeft() - params.leftMargin;//组件在容器X轴上的起点，需要注意，如果用户设置了left方向的Margin值，
                // 需要在取得itemViewleft属性后，将该margin抵消掉，因为，用户设置margin的意图明显不是想让分割线覆盖掉的
